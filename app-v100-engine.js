...
/* ---------- goal stages ---------- */
activeStage=function(date=todayDate()){
 if(date<=EXAM9)return{key:'nine',title:'9월 모의평가 대비',target:EXAM9,goals:GOAL9,label:'중간 체크포인트'};
 if(date<=V10_OCT_EXAM)return{key:'oct',title:'10월 학력평가 대비',target:V10_OCT_EXAM,goals:{국어:1,수학:1,영어:1,사회문화:1,경제:1},label:'중간 체크포인트'};
 return{key:'csat',title:'수능 최종 대비',target:V10_CSAT,goals:GOAL_CSAT,label:'최종 목표'};
};
...
const baseScheduleV10Core=baseSchedule;
baseSchedule=function(date){
...
capacityHoursFor=function(date){
...
capacitySummaryV90=function(from=viewDate,to=deadlineDate()){
...
curriculumLoadV90=function(date=viewDate){
...
const addTaskV10Core=addTask;
addTask=function(date,t){if((Number(t?.minutes)||0)<=0&&(t?.taskKind==='lecture'||(t?.components||[]).some(c=>c.kind==='lecture'))){const e=estimateTaskMinutesV10(normalizeImportedTask({...t,id:t.id||uid()}),date);if(e.known)t={...t,minutes:e.minutes}}return addTaskV10Core(date,t)};

/* ---------- time-aware placement ---------- */
function blockAvailableMinutesV10(date,b){const cap=blockDuration(b),used=(b.taskIds||[]).map(id=>taskById(date,id)).filter(Boolean).reduce((s,t)=>{const e=estimateTaskMinutesV10(t,date);return s+(e.known?e.minutes:cap)},0);return Math.max(0,cap-used)}
autoPlaceTasks=function(date,newTasks){
 const blocks=ensureSchedule(date),free=blocks.filter(b=>b.selfStudy&&!b.locked&&!studyBlockViolatesSleep(b));let placed=0;
 for(const t of newTasks){if(free.some(b=>(b.taskIds||[]).includes(t.id)))continue;const need=estimateTaskMinutesV10(t,date);if(!need.known)continue;let left=need.minutes,usedAny=false;const lecture=t.taskKind==='lecture'||(t.components||[]).some(c=>c.kind==='lecture');
  for(const b of free){if(left<=0)break;if(lecture&&!b.device)continue;const room=blockAvailableMinutesV10(date,b);if(room<=0)continue;b.taskIds=b.taskIds||[];if(!b.taskIds.includes(t.id))b.taskIds.push(t.id);left-=room;usedAny=true}
  if(usedAny)placed++;
 }
...
const saveWeeklyCommandV10Core=saveWeeklyCommand;
saveWeeklyCommand=function(){saveWeeklyCommandV10Core();const start=mondayOf(viewDate),n=DB.weeklyNotes[start]||{};DB.decisionLog.unshift({date:viewDate,kind:'weekly-directive',appliesWeek:addDays(start,7),directive:deep(n),at:Date.now()});DB.decisionLog=DB.decisionLog.slice(0,200);saveDB();renderWeeklyCommand()};
const renderWeeklyCommandV10Core=renderWeeklyCommand;
renderWeeklyCommand=function(){renderWeeklyCommandV10Core();const box=$('#weeklyCommand');if(!box)return;const applies=addDays(mondayOf(viewDate),7),n=DB.weeklyNotes[mondayOf(viewDate)]||{};if(n.p1||n.p2||n.p3||n.stop)box.insertAdjacentHTML('beforeend',`<div class="directive-effect"><b>${applies.slice(5).replace('-','.')} 주부터 실제 계산에 반영</b><span>핵심에 적힌 과목은 우선순위가 올라가고, ‘줄이거나 중단’에 정확히 적힌 과정은 그 주 필요량에서 일시 제외됩니다.</span></div>`)};
...
const renderDashboardV10FinalCore=renderDashboard;
renderDashboard=function(){renderDashboardV10FinalCore();const load=curriculumLoadV90(viewDate),q=$('#dailyQuotaBySubject'),need=$('#todayNeedHours');if(need){if(load.unknown.length&&!(load.low||load.high))need.textContent='계산 불가';else if(load.unknown.length)need.textContent=`확정 ${load.todayLow.toFixed(1)}–${load.todayHigh.toFixed(1)}h +`;else need.textContent=`${load.todayLow.toFixed(1)}–${load.todayHigh.toFixed(1)}h`;}if(q){const known=load.bySubject.filter(x=>x.todayLow||x.todayHigh);q.innerHTML=known.length?known.sort((a,b)=>b.todayHigh-a.todayHigh).map(x=>`<div class="quota-row"><b>${esc(x.subject)}</b><span><i style="width:${Math.min(100,load.todayHigh?x.todayHigh/load.todayHigh*100:0)}%"></i></span><strong>${x.todayLow.toFixed(1)}–${x.todayHigh.toFixed(1)}h</strong></div>`).join(''):`<div class="empty-state">과정별 시간 근거가 아직 부족합니다. 직접 시간을 입력하거나 시간표 관측이 2회 이상 쌓이면 계산합니다.</div>`}const cap=$('#todayNeedCaption');if(cap)cap.textContent=load.unknown.length?`과정별 목표일 역산 · 실제 시간표 가용 ${load.todayCap.toFixed(1)}h · 시간 미정 ${load.unknown.length}개는 위 숫자에 포함되지 않음`:`과정별 목표일 역산 · 실제 시간표 가용 ${load.todayCap.toFixed(1)}h · 전 과정 시간 근거 연결`};

const renderSettingsV101Core=renderSettings;
renderSettings=function(){renderSettingsV101Core();renderSafetySnapshotsV10()};

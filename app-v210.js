...
renderDecisionBoardV10=function(){
 ...
 box.innerHTML=`<div class="decision-summary ${p.overload?'risk':''}"><b>계획 조정</b><span>${summary}</span></div><div class="decision-list">${p.rows.map(x=>`<article class="decision-row ${x.manual?'manual':''}"><div><b>${esc(x.subject)}</b><small>${esc(x.action)}</small><details><summary>근거</summary>${x.reasons.map(r=>`<p>${esc(r)}</p>`).join('')}</details></div><strong>${x.planned.toFixed(1)}h 계획</strong><button class="text-link decision-pin" data-sub="${esc(x.subject)}">${x.manual?'우선 해제':'오늘 우선'}</button></article>`).join('')}</div><details class="y21-checks"><summary>학습 점검 · 미해결 ${p.rows.reduce((s,x)=>s+x.check.pending.length,0)}건</summary>${p.rows.map(x=>`<div class="y21-check-row"><b>${x.subject}</b><span>미해결 ${x.check.pending.length} · 원인 미확인 ${x.check.unanalysed.length}${x.check.recurring.length?` · 재발 ${x.check.recurring.length}`:''}</span><button class="btn ghost small" data-y21-review="${esc(x.subject)}">점검</button></div>`).join('')}</details>`;
...
function y21Merge(incoming,current,choices={}){
 ...
 const conflicts=[],restoredTasks=new Set();let additions=0;
 const deleted=new Set(Object.keys(current.deletedTasks||{}));
 for(const item of current.trash||[])if(item.type==='task'&&item.data?.id)deleted.add(item.data.id);
 for(const rows of Object.values(current.tasks||{}))for(const t of rows)deleted.delete(t.id);
 for(const [date,rows] of Object.entries(incoming.tasks||{}))if(Array.isArray(rows))incoming.tasks[date]=rows.filter(t=>{
  if(!deleted.has(t.id))return true;
  const path=['tasks',date,'@'+t.id],id=JSON.stringify(path);conflicts.push({id,path:path.join(' / ')+' · 삭제된 할 일',incoming:t,current:null});
  if(choices[id]==='incoming'){restoredTasks.add(t.id);return true;}return false;
 });
 ...
 function merge(a,b,path){if(a===undefined)return deep(b);if(b===undefined){additions++;return deep(a)}if(JSON.stringify(a)===JSON.stringify(b))return deep(b);
  ...
  if(Array.isArray(a)&&Array.isArray(b)){
   const key=x=>x&&typeof x==='object'?(path.at(-1)==='questionRecords'&&x.subject&&x.number?`${x.subject}:${x.number}`:(x.id??x.key??x.archiveId??x.packageId)):typeof x==='string'?x:null;
 ...
 const data=merge(incoming,current,[]);
 if(restoredTasks.size){data.trash=(data.trash||[]).filter(x=>!(x.type==='task'&&restoredTasks.has(x.data?.id)));for(const id of restoredTasks)if(data.deletedTasks)delete data.deletedTasks[id];}
 return{data,conflicts,additions};
}
function y21CommitMerge(incoming,choices={}){
 if(!validateBackupObject(incoming))throw new Error('지원하지 않거나 안전하지 않은 백업입니다.');
 const result=y21Merge(incoming,DB,choices);
 return commitChange(()=>{DB=migrateDB(result.data);});
}
...
__impl_initPwaUpdate=function(){
 if(!('serviceWorker'in navigator)){renderVersionStatus();return;}
 let registration,applying=false;
 ...
 navigator.serviceWorker.register('./sw-v200.js?v=2500').then(reg=>{
  ...
  reg.update().catch(()=>{});
 }).catch(()=>{globalThis.YEOKSANG_SW_VERSION=null;renderVersionStatus();});
 if($('#reloadUpdate'))$('#reloadUpdate').onclick=()=>{if(globalThis.YEOKSANG_FAILED_RAW){alert('미저장 기록을 먼저 내보내 주세요.');return;}if(registration?.waiting){applying=true;registration.waiting.postMessage({type:'APPLY_UPDATE'});}else location.reload();};
};

...
__impl_initPwaUpdate=function(){if(!('serviceWorker'in navigator))return;navigator.serviceWorker.register('./sw-v200.js?v=2100').then(r=>r.update()).catch(()=>{});navigator.serviceWorker.addEventListener('controllerchange',()=>$('#updateBanner')?.classList.remove('hidden'));if($('#reloadUpdate'))$('#reloadUpdate').onclick=()=>location.reload()};
__impl_renderVersionStatus=()=>{$('#runtimeStatus').textContent='2.1 · SW 2.1'};
const y21Settings=__impl_renderSettings;
__impl_renderSettings=function(){y21Settings();$('#versionInfo').innerHTML=`<code>曆象 2.1<br>Build ${BUILD}<br>Data schema ${SCHEMA_VERSION}<br>개인 기록은 이 브라우저에 저장</code>`};

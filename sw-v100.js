const VERSION='10.1';
const CACHE='project-11122-v101-20260907';
const ASSETS=['./','./index.html?v=1010','./styles-v100.css?v=1010','./personal-seed-v100.js?v=1010','./app-v100-core.js?v=1010','./app-v100-engine.js?v=1010','./manifest-v100.webmanifest?v=1010','./icon-192.png','./icon-512.png'];
...
self.addEventListener('fetch',event=>{
 ...
 event.respondWith(fetch(event.request,{cache:'no-store'}).then(response=>{if(response&&response.ok){const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(event.request,copy)).catch(()=>{})}return response}).catch(async()=>{const cached=await caches.match(event.request);if(cached)return cached;if(event.request.mode==='navigate')return (await caches.match('./index.html?v=1010'))||(await caches.match('./'));return Response.error()}))
});

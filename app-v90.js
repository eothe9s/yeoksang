/* 曆象 · PROJECT 11122 v9.0 · EXECUTION × INTELLIGENCE */
'use strict';
...
function normalizeQuestionRecord(q,fallback={}){
...
function normalizeTestRecord(t){
...
function normalizeDBShape(d){
 ...
 ['tasks','schedules','scheduleModes','scheduleHidden','lectureState','bookState','automationSkips','automationRuns','condition','plannerMeta','studyOverrides','planLocks','dailyRecords','meta'].forEach(k=>{if(!d[k]||typeof d[k]!=='object'||Array.isArray(d[k]))d[k]={}});
...
/* ============================================================
   v9.0 extension · execution first, intelligence second
   ============================================================ */
...
const normalizeQuestionRecordV8=normalizeQuestionRecord;
normalizeQuestionRecord=function(q,fallback={}){
...
function capacityHoursFor(date){
...
function capacitySummaryV90(from=viewDate,to=deadlineDate()){
...
function curriculumLoadV90(date=viewDate){
...
const navigateV8=navigate;
navigate=function(page){navigateV8(page);const group=page==='month'||page==='week'?'planner':page==='progress'?'vending':page==='condition'||page==='goals'||page==='settings'||page==='hike'?'analysis':page;$$('#mainNav button').forEach(b=>b.classList.toggle('active',b.dataset.page===group));const labels={dashboard:'오늘',planner:'계획',month:'월간',week:'주간',vending:'학습',progress:'진도·종료 기준',tests:'시험',analysis:'분석',condition:'수면·컨디션',goals:'목표·진단',settings:'설정·백업'};$('#pageTitle').textContent=labels[page]||'曆象'};

const renderDashboardV8=renderDashboard;
...
const saveBlockModalV8=saveBlockModal;
saveBlockModal=function(){
...
const openSleepModalV8=openSleepModal;
openSleepModal=function(date,time=''){openSleepModalV8(date,time);const sessionDate=addDays(date,1);if(!sleepSession(sessionDate).bed)$('#sleepBed').value=time||'';if(!sleepSession(sessionDate).wake)$('#sleepWake').value='';$('#sleepSummaryText').textContent=isWeekend(sessionDate)?'주말 기상 시각은 아직 고정하지 않습니다. 취침은 23:00 전입니다.':'평일 취침 23:00 전 · 기상 06:30 전'};
const saveSleepModalV8=saveSleepModal;
saveSleepModal=function(){const date=$('#sleepDate').value||viewDate,sessionDate=addDays(date,1),bed=$('#sleepBed').value,wake=$('#sleepWake').value;if(bed&&timeToMin(bed)>=23*60){alert('계획상 취침은 23:00보다 빨라야 합니다. 실제 위반 기록은 수면·컨디션 화면에서 남겨주세요.');return}if(!isWeekend(sessionDate)&&wake&&timeToMin(wake)>=6*60+30){alert('평일 계획 기상은 06:30보다 빨라야 합니다.');return}return saveSleepModalV8()};

function courseRowInfo(id,kind){const row=curriculumRowsV90().find(x=>x.id===id&&(!kind||x.kind===kind));return row||null}
function masteryChips(row){const m=row.meta?.mastery||{},learned=row.total!=null&&row.done>=row.total;return `<div class="mastery-chips"><span class="${learned?'on':''}">수강</span><span class="${m.review?'on':''}">복습</span><span class="${m.apply?'on':''}">적용</span><span class="${m.timed?'on':''}">시간 내 재현</span></div>`}
function openCourseMeta(id,kind){const row=courseRowInfo(id,kind);if(!row)return;const m=DB.courseMeta[id]||{};$('#courseMetaId').value=id;$('#courseMetaKind').value=kind||row.kind;$('#courseMetaTitle').textContent=`${row.subject} · ${row.name}`;$('#courseMetaLow').value=m.minutesPerUnitLow??m.minutesLow??'';$('#courseMetaHigh').value=m.minutesPerUnitHigh??m.minutesHigh??'';$('#courseMetaTarget').value=m.targetDate||deadlineDate();$('#courseMetaIncluded').checked=m.included!==false;$('#masteryReview').checked=Boolean(m.mastery?.review);$('#masteryApply').checked=Boolean(m.mastery?.apply);$('#masteryExplain').checked=Boolean(m.mastery?.explain);$('#masteryTimed').checked=Boolean(m.mastery?.timed);$('#courseMetaNote').value=m.note||'';showModal('courseMetaModal')}
function saveCourseMeta(){const id=$('#courseMetaId').value;if(!id)return;const low=$('#courseMetaLow').value,high=$('#courseMetaHigh').value;DB.courseMeta[id]={...(DB.courseMeta[id]||{}),minutesPerUnitLow:low===''?null:Number(low),minutesPerUnitHigh:high===''?null:Number(high),targetDate:$('#courseMetaTarget').value||deadlineDate(),included:$('#courseMetaIncluded').checked,mastery:{review:$('#masteryReview').checked,apply:$('#masteryApply').checked,explain:$('#masteryExplain').checked,timed:$('#masteryTimed').checked},note:$('#courseMetaNote').value.trim()};saveDB();hideModal('courseMetaModal');renderProgress();if($('#dashboard').classList.contains('active'))renderDashboard()}
...
const renderTestsV8=renderTests;
renderTests=function(){renderTestsV8();renderStabilityBoard();renderProtocolBoard()};

const openTestModalV8=openTestModal;
openTestModal=function(){openTestModalV8();['#testAbandonedQuestions','#testTimeLeft','#testFirstChoke','#testLastNormal','#testTimingMemo'].forEach(s=>{if($(s))$(s).value=''})};
...
openTestReviewModal=function(testId){
...
function renderErrorPatterns(){const box=$('#errorPatternBoard');if(!box)return;const rows=errorPatternStats();box.innerHTML=rows.length?rows.map(x=>`<div class="pattern-stat"><b>${esc(x.pattern)}</b><span>${x.count}회 · 미해결 ${x.pending} · ${[...x.subjects].join(' / ')}</span><small>${[...x.rules].slice(0,2).map(esc).join(' · ')||'통제 규칙 미입력'}</small></div>`).join(''):'<div class="empty-state">시험 문항 분석에서 오류 패턴을 기록하면 번호가 달라도 같은 행동을 묶어 보여줍니다.</div>'}
const renderAnalysisV8=renderAnalysis;
renderAnalysis=function(){renderAnalysisV8();renderWeeklyCommand();renderReadiness();renderErrorPatterns()};

const renderConditionV8=renderCondition;
renderCondition=function(){renderConditionV8();const d=$('#conditionDate').value||viewDate,c=DB.condition[d]||{},g=sleepBoundary(d);if($('#exerciseMinutes'))$('#exerciseMinutes').value=c.exerciseMinutes??'';if($('#exerciseIntensity'))$('#exerciseIntensity').value=c.exerciseIntensity||'';if($('#urgencyScore'))$('#urgencyScore').value=c.urgency??'';const box=$('#conditionGuardSummary');if(box)box.innerHTML=`<b>${g.status==='ok'?'수면 경계 준수':g.status==='violation'?'수면 경계 위반 기록':'수면 경계 기록 필요'}</b><span>${g.weekend?'주말 기상 목표 미정 · ':''}${g.sleep!=null?`실제 수면 ${minuteLabel(g.sleep)}`:'실제 수면시간 미계산'}</span>`};
const saveConditionV8=saveCondition;
saveCondition=function(){const d=$('#conditionDate').value;saveConditionV8();DB.condition[d]={...(DB.condition[d]||{}),exerciseMinutes:Number($('#exerciseMinutes')?.value)||0,exerciseIntensity:$('#exerciseIntensity')?.value||'',urgency:$('#urgencyScore')?.value??''};saveDB();renderCondition();if(d===viewDate)renderDashboard()};

const openLectureModalV8=openLectureModal;
openLectureModal=function(){openLectureModalV8();if($('#lectureMinutesLow'))$('#lectureMinutesLow').value='';if($('#lectureMinutesHigh'))$('#lectureMinutesHigh').value='';if($('#newMaterialGuard'))$('#newMaterialGuard').textContent='시간을 입력하면 10월 16일 계획에 미치는 영향을 계산합니다.'};
...
saveLectureModal=function(){const name=$('#lectureName').value.trim(),provider=$('#lectureProvider').value.trim(),total=Number($('#lectureTotal').value);if(!name||!provider||!Number.isInteger(total)||total<1){alert('강사·강좌명·전체 강의 수를 확인하세요.');return}const low=$('#lectureMinutesLow')?.value,high=$('#lectureMinutesHigh')?.value,key='custom-'+uid();DB.customLectures.push({key,subject:$('#lectureSubject').value,provider,series:$('#lectureSubject').value,name,display:name,total,custom:true});DB.courseMeta[key]={minutesPerUnitLow:low===''?null:Number(low),minutesPerUnitHigh:high===''?null:Number(high||low),targetDate:deadlineDate(),included:true,mastery:{}};saveDB();hideModal('lectureModal');renderVending();renderProgress()};

...
function dayTaskLoadV10(date){const arr=(DB.tasks[date]||[]).filter(t=>!t.done&&!t.deferred),known=arr.reduce((s,t)=>{const e=estimateTaskMinutesV10(t,date);return s+(e.known?e.minutes:0)},0),unknown=arr.filter(t=>!estimateTaskMinutesV10(t,date).known).length,cap=Math.round(capacityHoursFor(date)*60);return{known,unknown,cap,ratio:cap?known/cap:known?99:0}}
function bestCarryDateV10(fromDate,t){
 const need=estimateTaskMinutesV10(t,fromDate);if(!need.known||need.minutes<=0)return null;
 let best=null;
 for(let i=1;i<=7;i++){
  const date=addDays(fromDate,i),load=dayTaskLoadV10(date);if(load.cap-load.known<need.minutes)continue;
  const oldTasks=DB.tasks[date],oldSchedule=DB.schedules[date],depth=SAVE_TRANSACTION_DEPTH;
  let fits=false;
  try{SAVE_TRANSACTION_DEPTH++;DB.tasks[date]=deep(oldTasks||[]);DB.schedules[date]=deep(oldSchedule||[]);const candidate={...deep(t),id:uid(),done:false};delete candidate.deferred;DB.tasks[date].push(candidate);fits=autoPlaceTasks(date,[candidate])===1;}
  finally{SAVE_TRANSACTION_DEPTH=depth;if(oldTasks===undefined)delete DB.tasks[date];else DB.tasks[date]=oldTasks;if(oldSchedule===undefined)delete DB.schedules[date];else DB.schedules[date]=oldSchedule;}
  if(!fits)continue;const score=(load.known+need.minutes)/load.cap+i*.025;if(!best||score<best.score)best={date,score};
 }
 return best?.date||null;
}
function recommendUnfinishedActionV10(t,date,best=bestCarryDateV10(date,t)){const directive=effectiveWeeklyDirectiveV10(addDays(date,1)),stop=String(directive.stop||'').toLowerCase();if(stop&&[t.name,t.material].filter(Boolean).some(x=>stop.includes(String(x).toLowerCase())))return'waiting';if(t.priority==='extra')return'skip';if(!best)return'waiting';return t.priority==='must'&&best===nextDate(date)?'tomorrow':'spread'}
...
function openCloseDay__impl2(){
 ...
 const unfinished=tasksFor(viewDate).filter(t=>!t.done&&!t.deferred);$('#closeUnfinished').innerHTML=unfinished.length?`<h3 class="subhead">미완료 ${unfinished.length}개</h3><div class="micro-label">내일로 몰아넣지 않고 중요도와 다음 7일 여유를 기준으로 기본값을 추천합니다.</div>`+unfinished.map(t=>{const best=bestCarryDateV10(viewDate,t),rec=recommendUnfinishedActionV10(t,viewDate,best);return`<div class="close-choice"><div><b>${esc(t.subject)} · ${esc(t.name)}</b><div class="task-meta">${esc(t.material||'')} · 추천 ${rec==='tomorrow'?'내일':rec==='spread'?(best?`${best.slice(5)} 재배치`:'배치 가능한 날 없음 · 대기'):rec==='waiting'?'대기함':'건너뛰기'}</div></div><select class="input close-action" data-id="${esc(t.id)}"><option value="tomorrow"${rec==='tomorrow'?' selected':''}>내일</option><option value="spread"${rec==='spread'?' selected':''}>이번 주 여유일로 재배치</option><option value="waiting"${rec==='waiting'?' selected':''}>대기함</option><option value="skip"${rec==='skip'?' selected':''}>건너뛰기</option></select></div>`}).join(''):'<div class="muted">미완료 할 일이 없습니다.</div>';showModal('closeDayModal');
};
__impl_confirmCloseDay=confirmCloseDay__impl2;
function confirmCloseDay__impl2(){
 const previous=dailyRecord(viewDate);
 if(previous?.closedAt&&!confirm('이미 마감한 날짜입니다. 현재 상태로 다시 저장할까요?'))return;
 const rawStudy=$('#closeStudyOverride').value,v=Number(rawStudy);
 if(rawStudy!==''&&(!Number.isFinite(v)||v<0||v>24)){alert('순공 시간은 0~24시간으로 입력하세요.');return}
 const actions=$$('.close-action').map(x=>({id:x.dataset.id,act:x.value}));
 const ok=commitChange(()=>{
  retainDailyBasis(viewDate);
  if(rawStudy!=='')DB.studyOverrides[viewDate]=Math.round(v*60);
  for(const action of actions){
   const t=taskById(viewDate,action.id);if(!t||t.done||t.deferred)continue;
   if(action.act==='waiting'){moveTaskToWaiting(viewDate,t.id);continue;}
   if(action.act==='skip'){t.deferred={kind:'skip',units:taskUnitSnapshot(t),at:Date.now()};continue;}
   const target=action.act==='tomorrow'?nextDate(viewDate):bestCarryDateV10(viewDate,t);
   if(!target){moveTaskToWaiting(viewDate,t.id);continue;}
   const units=taskUnitSnapshot(t),copy=deep(t);copy.origin={date:viewDate,id:t.id};
   const c=carryTaskToDate(copy,target);t.deferred={kind:'carry',units,targetDate:target,at:Date.now()};
   autoPlaceTasks(target,[c]);DB.decisionLog.unshift({date:viewDate,kind:'carry',task:t.name,from:viewDate,to:target,mode:action.act,at:Date.now()});
  }
  const totals=dailyCompletion(viewDate),record=DB.dailyRecords[viewDate];
  Object.assign(record,totals,{state:'closed',study:finalStudy(viewDate),closedAt:Date.now()});
  DB.closeHistory=DB.closeHistory.filter(x=>x.date!==viewDate);DB.closeHistory.unshift({date:viewDate,at:Date.now(),study:record.study,completion:deep(record)});
  DB.closeHistory=DB.closeHistory.slice(0,50);DB.decisionLog=DB.decisionLog.slice(0,200);
 });
 if(!ok)return;
 hideModal('closeDayModal');renderDashboard();const saved=dailyCompletion(viewDate);alert('오늘 마감을 저장했습니다. 완주 '+saved.done+'/'+saved.total);
};
...
function ensureDailySafetySnapshotV10(){/* v2.5: full localStorage snapshots disabled; existing copies preserved. */}

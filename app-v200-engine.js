...
function ensureDailySafetySnapshotV10(){if(globalThis.YEOKSANG_STARTING)return;const key=todayDate(),arr=dailySafetySnapshotsV10();if(arr.some(x=>x.date===key)||approximateStorageBytes()>2.7*1024*1024)return;arr.unshift({date:key,at:Date.now(),data:LAST_SAVED_JSON});try{localStorage.setItem('p11122_v100_daily_safety',JSON.stringify(arr.slice(0,3)))}catch{}}

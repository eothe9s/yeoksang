...
vm.createContext(ctx);vm.runInContext(fs.readFileSync(__dirname+'/app-v110-core.js','utf8'),ctx);vm.runInContext(fs.readFileSync(__dirname+'/app-v110-engine.js','utf8'),ctx);vm.runInContext(fs.readFileSync(__dirname+'/app-v110-recovery.js','utf8'),ctx);
...
if(!process.exitCode)console.log('ALL LEGACY + V11 CORE TESTS PASSED');
...
console.log('ALL V11 END-TO-END TESTS PASSED');

test('v11 public function surface has no same-name override chain',()=>{
 const src=['app-v110-core.js','app-v110-engine.js','app-v110-recovery.js'].map(f=>fs.readFileSync(__dirname+'/'+f,'utf8')).join('\n');

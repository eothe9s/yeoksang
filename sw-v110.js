const VERSION='11.0';
const CACHE='yeoksang-v11-20260907';
const ASSETS=['./','./index.html?v=1100','./styles-v110.css?v=1100','./baseline-v110.js?v=1100','./app-v110-core.js?v=1100','./app-v110-engine.js?v=1100','./app-v110-recovery.js?v=1100','./manifest-v110.webmanifest?v=1100','./icon-192.png','./icon-512.png'];
...
self.addEventListener('fetch',event=>{
 ...
 event.respondWith(fetch(event.request,{cache:'no-store'}).then(response=>{if(response&&response.ok){const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(event.request,copy)).catch(()=>{})}return response}).catch(async()=>{const cached=await caches.match(event.request);if(cached)return cached;if(event.request.mode==='navigate')return (await caches.match('./index.html?v=1100'))||(await caches.match('./'));return Response.error()}))
});

const VERSION='2.0';
const CACHE='yeoksang-v2-0-20260907';
const ASSETS=['./','./index.html?v=2000','./styles-v200.css?v=2000','./baseline-v200.js?v=2000','./app-v200-core.js?v=2000','./app-v200-engine.js?v=2000','./app-v200-recovery.js?v=2000','./manifest-v200.webmanifest?v=2000','./icon-192.png','./icon-512.png'];
...
self.addEventListener('fetch',event=>{
 ...
 event.respondWith(fetch(event.request,{cache:'no-store'}).then(response=>{if(response&&response.ok){const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(event.request,copy)).catch(()=>{})}return response}).catch(async()=>{const cached=await caches.match(event.request);if(cached)return cached;if(event.request.mode==='navigate')return (await caches.match('./index.html?v=2000'))||(await caches.match('./'));return Response.error()}))
});

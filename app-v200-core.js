...
function loadDB(){
 ...
 try{stored=localStorage.getItem(DB_KEY);if(stored!==null){raw=JSON.parse(stored);if(!raw||typeof raw!=='object'||Array.isArray(raw))throw new Error('invalid saved data')}}catch(e){
  document.addEventListener('DOMContentLoaded',()=>{globalThis.y211ShowRecovery?.('기존 저장값을 읽지 못했습니다. 초기화하지 않고 중단했습니다.');});
 ...
 // Keep original bytes in memory. An extra full localStorage copy can exhaust quota
 // and must never be a prerequisite for opening the existing database.
 globalThis.YEOKSANG_ORIGINAL_RAW=stored;
...
function renderTestAllSubjectSummary(){
 const box=$('#testAllSubjectSummary');if(!box)return;const represented=SUBJECTS.filter(s=>latestRepresentative(s)).length,pending=reviewEntries({}).length,comparable=SUBJECTS.filter(s=>questionPattern(s,{scope:'representative'}).attempts>=3).length;
...
function renderQuestionPattern(subject,filters){
 ...
 const history=[...stat.entries].sort((a,b)=>(b.test._patternOrder||b.test.date).localeCompare(a.test._patternOrder||a.test.date));insight.innerHTML=`<div class="question-focus"><b>${active}번 · ${stat.wrongTestIds.size}/${pattern.attempts}회 오답${stat.uncertainTestIds.size?` · 애매 정답 ${stat.uncertainTestIds.size}회`:''}</b><span>${stat.wrongTestIds.size>=2?'반복 확인 문항입니다. 원인과 재풀이 결과를 남겨야 해결로 바뀝니다.':'다음 기록에서 반복 여부를 다시 확인합니다.'}</span></div><div class="question-history">${history.map(({test,question})=>`<div><b>${esc(test._patternLabel||`${test.date.slice(5).replace('-','.')} · ${testSourceLabel(test.source)}`)}</b><span>${QUESTION_STATUS_LABELS[question.status]}${question.type?` · ${esc(question.type)}`:''}${question.cause?` · ${esc(question.cause)}`:''} · ${reviewStatusLabel(question)}</span>${test._patternExamKey?`<button class="btn ghost small pattern-exam-open" data-exam-key="${esc(test._patternExamKey)}">분석</button>`:question.retryState!=='resolved'?`<button class="btn ghost small review-open" data-test-id="${esc(test.id)}">분석</button>`:''}</div>`).join('')}</div>`;
 $$('.pattern-exam-open').forEach(b=>b.onclick=()=>y24OpenExam(b.dataset.examKey))
}
...
function renderDashboard__impl2(){
 ...
 else if(guard.status==='violation'){title.textContent='수면 규칙 위반 기록';detail.textContent=`${guard.bed||'취침 미입력'} · ${guard.wake||'기상 미입력'}`;state.textContent='보정 필요';state.className='status-chip bad';mini.textContent='수면 경계 위반'}

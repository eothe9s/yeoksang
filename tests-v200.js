...
vm.createContext(ctx);vm.runInContext(fs.readFileSync(__dirname+'/app-v200-core.js','utf8'),ctx);vm.runInContext(fs.readFileSync(__dirname+'/app-v200-engine.js','utf8'),ctx);vm.runInContext(fs.readFileSync(__dirname+'/app-v200-recovery.js','utf8'),ctx);
...
test('schema 9 test records migrate without losing grades',()=>{
 ...
 assert.deepStrictEqual(plain(val('out')),{schema:14,source:'기타',grade:2,wrongType:'object'});
});
...
test('schema 10 migrates question strings to records',()=>{
 ...
 assert.deepStrictEqual(plain(val('out')),{schema:14,questions:[[2,'wrong'],[4,'uncertain'],[9,'wrong']]});
});
...
if(!process.exitCode)console.log('ALL LEGACY + V2 CORE TESTS PASSED');
...
try{
 ...
 test('v10.1 baseline merge upgrades existing DB without replacing history',()=>{vm.runInContext(fs.readFileSync(__dirname+'/baseline-v200.js','utf8'),ctx);exec(`DB=defaultDB();DB.tasks['2026-09-06']=[normalizeImportedTask({id:'keep',subject:'수학',name:'기존 기록',minutes:30,done:true})];DB.tests=[];DB.meta={};LAST_SAVED_JSON=JSON.stringify(DB);applyPersonalBaselineV101();globalThis.out={kept:DB.tasks['2026-09-06'][0].name,test:DB.tests.some(t=>t.id==='exam-2026-09-kice-full'),meta:!!DB.meta.personalBaselineV101Applied,core:DB.courseMeta['eco-core']?.minutesPerUnitLow}`);assert.deepStrictEqual(plain(val('out')),{kept:'기존 기록',test:true,meta:true,core:45})});
 test('v10.1 personal baseline migration is idempotent',()=>{vm.runInContext(fs.readFileSync(__dirname+'/baseline-v200.js','utf8'),ctx);exec(`DB=defaultDB();DB.tests=[];DB.meta={};LAST_SAVED_JSON=JSON.stringify(DB);applyPersonalBaselineV101();const first={tests:DB.tests.length,extra:DB.curriculumExtra.length,signals:DB.diagnosticSignals.length};applyPersonalBaselineV101();globalThis.out={first,second:{tests:DB.tests.length,extra:DB.curriculumExtra.length,signals:DB.diagnosticSignals.length}}`);const o=plain(val('out'));assert.deepStrictEqual(o.first,o.second)});
 test('v10.1 W.O.W reset never erases post-baseline activity',()=>{vm.runInContext(fs.readFileSync(__dirname+'/baseline-v200.js','utf8'),ctx);exec(`DB=defaultDB();DB.meta={};DB.lectureState['kor-wow::1']={completed:true};DB.tasks['2026-09-05']=[normalizeImportedTask({id:'wow-new',subject:'국어',name:'W.O.W 1강',components:[{id:'w',kind:'lecture',ref:'kor-wow::1',label:'01강',done:true}]})];LAST_SAVED_JSON=JSON.stringify(DB);applyPersonalBaselineV101();globalThis.out=!!DB.lectureState['kor-wow::1']?.completed`);assert.equal(plain(val('out')),true)});
}catch(e){console.error(e);process.exitCode=1}
if(!process.exitCode)console.log('ALL V2 BASELINE TESTS PASSED');


// v2 acceptance tests for the locked 43-item scope
try{
 test('v2 brand no longer uses PROJECT 11122 in primary shell',()=>{const html=fs.readFileSync(__dirname+'/index.html','utf8');assert.ok(html.includes('<h1>曆象</h1>'));assert.ok(html.includes('수능 만점 운영체제'));assert.ok(!html.includes('<h1>PROJECT 11122</h1>'))});
 test('v2 modals are fixed overlays and hidden by default',()=>{const css=fs.readFileSync(__dirname+'/styles-v200.css','utf8');assert.ok(/\.modal-back\{[^}]*display:none!important[^}]*position:fixed!important/s.test(css));assert.ok(/\.modal-back\.show\{display:flex!important/.test(css))});
 test('v2 main navigation has six locked sections',()=>{const html=fs.readFileSync(__dirname+'/index.html','utf8');const nav=(html.match(/<nav id="mainNav"[\s\S]*?<\/nav>/)||[''])[0];for(const x of ['오늘','시간표','학습','시험','분석','설정'])assert.ok(nav.includes('>'+x+'<'))});
 test('v2 90-minute flexible task allocates 50 plus 40, not 100',()=>{exec(`ensureV100DB();const d='2026-09-15',tpl=createScheduleTemplate('two50',[{name:'A',type:'self',selfStudy:true,device:true,start:'18:00',end:'18:50'},{name:'B',type:'self',selfStudy:true,device:true,start:'18:50',end:'19:40'}]);DB.scheduleModes[d]={mode:'template',templateId:tpl.id};DB.schedules[d]=[];DB.tasks[d]=[];const t=addTask(d,{subject:'수학',priority:'must',name:'90분',minutes:90,splitMode:'flex',components:[{id:'m',kind:'manual',label:'90분',done:false}]});autoPlaceTasks(d,[t]);const bs=ensureSchedule(d).filter(b=>(b.taskIds||[]).includes(t.id));globalThis.out=bs.map(b=>b.taskAllocations[t.id])`);assert.deepStrictEqual(plain(val('out')),[50,40])});
 test('v2 flexible allocation leaves ten minutes reusable in second block',()=>{exec(`ensureV100DB();const d='2026-09-15',tpl=createScheduleTemplate('two50',[{name:'A',type:'self',selfStudy:true,device:true,start:'18:00',end:'18:50'},{name:'B',type:'self',selfStudy:true,device:true,start:'18:50',end:'19:40'}]);DB.scheduleModes[d]={mode:'template',templateId:tpl.id};DB.schedules[d]=[];DB.tasks[d]=[];const a=addTask(d,{subject:'수학',priority:'must',name:'90분',minutes:90,splitMode:'flex',components:[{id:'a',kind:'manual',label:'a',done:false}]});autoPlaceTasks(d,[a]);const b=ensureSchedule(d).find(x=>x.name==='B');globalThis.out=blockAvailableMinutesV10(d,b)`);assert.equal(plain(val('out')),10)});
 test('v2 contiguous task refuses separated blocks',()=>{exec(`ensureV100DB();const d='2026-09-15',tpl=createScheduleTemplate('gap',[{name:'A',type:'self',selfStudy:true,device:true,start:'18:00',end:'18:50'},{name:'B',type:'self',selfStudy:true,device:true,start:'19:00',end:'19:50'}]);DB.scheduleModes[d]={mode:'template',templateId:tpl.id};DB.schedules[d]=[];DB.tasks[d]=[];const t=addTask(d,{subject:'수학',priority:'must',name:'90분 실모',minutes:90,splitMode:'contiguous',components:[{id:'m',kind:'manual',label:'90분',done:false}]});globalThis.out=autoPlaceTasks(d,[t])`);assert.equal(plain(val('out')),0)});
 test('v2 recovery merge preserves old history and newer current date',()=>{exec(`const old=defaultDB(),cur=defaultDB();old.tasks['2026-09-01']=[normalizeImportedTask({id:'old1',subject:'국어',name:'옛 기록',done:true})];old.tests=[normalizeTestRecord({id:'exam-old',kind:'single',date:'2026-09-01',subject:'국어',score:80})];cur.tasks['2026-09-07']=[normalizeImportedTask({id:'new1',subject:'수학',name:'오늘 기록',done:false})];const m=v11MergeDB(old,cur);globalThis.out={old:m.tasks['2026-09-01'].length,new:m.tasks['2026-09-07'].length,tests:m.tests.length}`);assert.deepStrictEqual(plain(val('out')),{old:1,new:1,tests:1})});
 test('v2 period average exposes calendar average separately from today quota',()=>{exec(`ensureV100DB();DB.curriculumExtra=[{id:'x',subject:'수학',name:'X',kind:'unit',totalUnits:10,completedUnits:0,minutesPerUnitLow:60,minutesPerUnitHigh:60,included:true,targetDate:'2026-10-16'}];DB.courseMeta={};DB.settings.curriculumDeadline='2026-10-16';globalThis.out=periodAverageNeedV11('2026-09-07')`);const o=plain(val('out'));assert.equal(o.days,40);assert.ok(Math.abs(o.low-.25)<.001);assert.ok(Math.abs(o.high-.25)<.001)});
 test('v2 service worker only points at v2 assets',()=>{const sw=fs.readFileSync(__dirname+'/sw-v200.js','utf8');assert.ok(sw.includes('yeoksang-v2-'));assert.ok(!sw.includes('styles-v100'));assert.ok(sw.includes('app-v200-recovery.js'))});
}catch(e){console.error(e);process.exitCode=1}
if(!process.exitCode)console.log('ALL V2 ACCEPTANCE TESTS PASSED');

test('v2 end-to-end recovery planning execution exam feedback loop',()=>{
 exec(`
  const d='2026-09-15',old=defaultDB();
  old.tasks[addDays(d,-1)]=[normalizeImportedTask({id:'hist',subject:'경제',name:'과거 기출',done:true,components:[{id:'hc',kind:'manual',label:'완료',done:true}]})];
  DB=v11MergeDB(old,defaultDB());
  const tpl=createScheduleTemplate('flow-two50',[{name:'자습1',type:'self',selfStudy:true,device:true,start:'18:00',end:'18:50'},{name:'자습2',type:'self',selfStudy:true,device:true,start:'18:50',end:'19:40'}]);DB.scheduleModes[d]={mode:'template',templateId:tpl.id};DB.schedules[d]=[];
  const task=addTask(d,{id:'flow',subject:'수학',priority:'must',name:'CONTACT',minutes:90,splitMode:'flex',components:[{id:'fc',kind:'manual',label:'CONTACT',done:false}]});
  autoPlaceTasks(d,[task]);const allocBefore=ensureSchedule(d).filter(b=>(b.taskIds||[]).includes('flow')).map(b=>b.taskAllocations?.flow||0);setTaskDoneInternal(d,'flow',true);
  const exam=normalizeTestRecord({id:'exam-flow',kind:'single',source:'단원·과목 실모',scope:'unit',date:d,name:'수학 실모',subject:'수학',score:88,wrongQuestions:'21',questionRecords:[normalizeQuestionRecord({subject:'수학',number:21,status:'wrong',pattern:'계산 절차 누락',retryDue:addDays(d,1),retryState:'pending',controlRule:'검산 10초'})]});
  DB.tests.push(exam);DB.weeklyNotes[mondayOf(d)]={p1:'수학',p2:'',p3:'',stop:'',updatedAt:Date.now()};
  const patterns=errorPatternStats();
  globalThis.out={history:(DB.tasks[addDays(d,-1)]||[]).length,done:DB.tasks[d][0].done,alloc:allocBefore,pattern:patterns[0]?.pattern,priority:weeklySubjectBoostV10('수학',addDays(mondayOf(d),7)).score};
 `);
 assert.deepStrictEqual(plain(val('out')),{history:1,done:true,alloc:[50,40],pattern:'계산 절차 누락',priority:3});
});
console.log('ALL V2 END-TO-END TESTS PASSED');

test('v2 public function surface has no same-name override chain',()=>{
 const src=['app-v200-core.js','app-v200-engine.js','app-v200-recovery.js'].map(f=>fs.readFileSync(__dirname+'/'+f,'utf8')).join('\n');
 const names=[...src.matchAll(/^function\s+([A-Za-z_$][\w$]*)\s*\(/gm)].map(m=>m[1]);
 const seen=new Set();for(const n of names){assert.ok(!seen.has(n),'duplicate function '+n);seen.add(n)}
 assert.equal((src.match(/^[A-Za-z_$][\w$]*\s*=\s*function\s*\(/gm)||[]).length,0);
});
console.log('ALL V2 STRUCTURE TESTS PASSED');

test('v2.0 removes prototype-like helper copy from primary screens',()=>{
...
test('v2.0 timetable is compact on iPad landscape',()=>{
 const css=fs.readFileSync(__dirname+'/styles-v200.css','utf8');
 assert.ok(css.includes('grid-template-columns:420px 218px!important'));
 assert.ok(css.includes('grid-template-columns:42px repeat(6,63px)!important'));
 assert.ok(css.includes('@media(max-width:1180px) and (min-width:821px)'));
});
test('v2.0 uses a light quiet sidebar instead of the black rail',()=>{
 const css=fs.readFileSync(__dirname+'/styles-v200.css','utf8');
 const html=fs.readFileSync(__dirname+'/index.html','utf8');
 assert.ok(css.includes('linear-gradient(180deg,#f1f4f8 0%,#eaf0f6 100%)'));
 assert.ok(css.includes('--v2-sidebar:156px'));
 assert.ok(html.includes('<h1>曆象</h1><span>v2.0</span>'));
});
test('v2.0 restores a soft gradient final-goal card',()=>{
 const css=fs.readFileSync(__dirname+'/styles-v200.css','utf8');
 assert.ok(css.includes('background:linear-gradient(118deg,#ffffff 0%,#f5f8fc 43%,#eef3fb 72%,#fbf1f5 100%)'));
});
test('v2.0 today execution exposes four primary decisions, not six equal boxes',()=>{
 const html=fs.readFileSync(__dirname+'/index.html','utf8');
 const block=(html.match(/<div class="hero-metrics v2-metrics">[\s\S]*?<\/div>\n        <div class="hero-subline v2-subline">/)||[''])[0];
 assert.equal((block.match(/<div class="hero-metric(?: |")/g)||[]).length,4);
 assert.ok(block.includes('오늘 마감 필요량'));
 assert.ok(block.includes('오늘 실제 최대 가용'));
 assert.ok(block.includes('계획 부족 / 초과'));
 assert.ok(block.includes('지금 할 것'));
});

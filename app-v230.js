...
function y230DeleteArchiveQuestion(archiveId,key){
 const t=(DB.analysisArchive||[]).find(x=>x.archiveId===archiveId);if(!t)return;const i=(t.questions||[]).findIndex(q=>y230QuestionKey(q)===key);if(i<0)return;if(!confirm('이 문항 분석을 휴지통으로 이동할까요?'))return;const q=t.questions[i];trashPush('analysisArchiveQuestion',{archiveId,question:deep(q),index:i,examLabel:t.examLabel});t.questions.splice(i,1);y230RebuildArchiveDerived();saveDB();renderTests();renderAnalysis();
}
function y230DeleteArchiveTest(archiveId){
 const t=(DB.analysisArchive||[]).find(x=>x.archiveId===archiveId);if(!t)return;if(!confirm(`${t.examLabel||'과거 시험'} 기록을 휴지통으로 이동할까요?`))return;trashPush('analysisArchiveTest',{test:deep(t)});DB.analysisArchive=DB.analysisArchive.filter(x=>x.archiveId!==archiveId);y230RebuildArchiveDerived();saveDB();renderTests();renderAnalysis();
}
function y230DeletePackage(packageId){
 ...
 const bundle={packageId,importRow:deep(imp),archiveTests:deep((DB.analysisArchive||[]).filter(x=>x.packageId===packageId)),activeFragments};
 trashPush('analysisPackage',bundle);DB.analysisArchive=(DB.analysisArchive||[]).filter(x=>x.packageId!==packageId);DB.analysisImports=(DB.analysisImports||[]).filter(x=>x.packageId!==packageId);y230RebuildArchiveDerived();saveDB();renderTests();renderAnalysis();
}
...
renderDecisionBoardV10=function(){
 ...
 box.innerHTML=`<div class="decision-summary ${p.overload?'risk':''}"><b>${p.reactivation?'재가동':'계획 조정'}</b><span>${summary}</span></div><div class="decision-list">${p.rows.map(x=>`<article class="decision-row ${x.manual?'manual':''}"><div><b>${esc(x.subject)}</b><small>${esc(x.action)}</small><details><summary>근거</summary>${x.reasons.map(r=>`<p>${esc(r)}</p>`).join('')}</details></div><strong>${x.planned.toFixed(1)}h 계획</strong><button class="text-link decision-pin" data-sub="${x.subject}">${x.manual?'우선 해제':'오늘 우선'}</button></article>`).join('')}</div><details class="y21-checks"><summary>학습 점검 · 현재 미해결 ${p.rows.reduce((s,x)=>s+x.check.pending.length,0)}건</summary>${p.rows.map(x=>`<div class="y21-check-row"><b>${x.subject}</b><span>미해결 ${x.check.pending.length}${x.check.recurring.length?` · 재발 ${x.check.recurring.length}`:''}</span><button class="btn ghost small" data-y21-review="${x.subject}">점검</button></div>`).join('')}</details>`;

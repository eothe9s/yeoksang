...
function y220ApplyPackage(){
 ...
 // External export above is the safety copy; do not duplicate the entire DB in localStorage.
...
function y220ExportContext(){
 ...
 const context={kind:Y220_CONTEXT_KIND,schemaVersion:1,createdAt:new Date().toISOString(),appVersion:'2.4',viewDate,subjects:deep(SUBJECTS),tests,analysisArchive:archive,analysisObservations:deep(DB.analysisObservations||[]),analysisPatterns:deep(DB.analysisPatterns||[]),analysisImports:deep(DB.analysisImports||[]),subjectProtocols:deep(DB.subjectProtocols||{}),coverageTopics:deep(DB.coverageTopics||{}),coverageChecks:deep(DB.coverageChecks||{}),courseMeta:deep(DB.courseMeta||{}),weeklyNotes:deep(DB.weeklyNotes||{})};
 y220Download(`YEOKSANG_ANALYSIS_CONTEXT_${todayDate()}.json`,context)
}
function y220Rollback(){const id=DB.meta?.lastAnalysisImport?.packageId;if(!id||(DB.analysisImports||[]).every(x=>x.packageId!==id)){alert('삭제할 최근 분석 패키지가 없습니다. 기존 안전본은 설정에서 병합할 수 있습니다.');return}y230DeletePackage(id);}
